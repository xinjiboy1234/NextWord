...
> 文档版本：2026-06-27  
> 状态：**设想阶段**（架构原则与方向性记录，非实现规格）  
> 用途：记录从「AI 驱动的学习体验需求」到「Score 内核 + CEFR 映射层」的完整来龙去脉，供后续 spec 与实现计划引用。  
> 相关文档：[CURRENT-STATE.md](./CURRENT-STATE.md)、[PLAN-Overview.md](../plans/PLAN-Overview.md)
...
这与 `plans/PLAN-Overview.md` 已有原则一致：
...
## 9. 与现有 PLAN-Overview 的关系

`PLAN-Overview.md` 当前写法：
...
1. 产品确认 §10 待决问题  
2. 撰写 implementation spec（数据模型、API 字段、迁移阶段）  
3. 更新 `plans/PLAN-Overview.md` 分级原则段  
4. 按 P0→P4 拆分：阅读查词真实 LLM → 评价报告 → 工具层 → 每日词 → 搜索  
