...
- **生命周期证据（T-014）**：指定词造句评分后按目标词使用情况推进/回退——句中含目标词（统一命中口径 T-040：单词词边界、多词短语连续词序列）且 A/B 档 → 确认 prompted use（待自发）；含目标词但 D 档或词汇维 ≤2 → 退回回忆阶段重进 SM-2 调度；指定目标词的产出永远不算自发。自由表达（非指定目标词）评分达标时，文中自发出现的 prompted_use 候选池词毕业（spontaneous_use）并留痕 `GraduatedFreeExpressionLogId`——**毕业评分口径（T-044 放宽）**：整篇 C 及以上且词汇维 ≥3 即达标（D 档或词汇维 ≤2 不毕业，防烂底线不动；造句确认仍要求 A/B）。
...
- 端点：`GET /api/assessment/{id}/next-block`（幂等，未提交的块重发原题；**T-065：本块已提交、后台评分中时返回 `evaluating=true` 不重发题目**）→ `POST /api/assessment/{id}/blocks/{n}/submit`（**T-065 异步评分：先存答案 + 标记 pending-scoring + 入队 `AssessmentBlockScoring` 后台任务 → 202 立即返回，评分在后台进行（`ScoreBlockJobAsync`：评分核心 + 收敛 `FinalizeAsync` + 报告/计划入队），前端轮询 next-block 衔接**）。**T-064/T-070：首次测评（未完成过）前检查 `GET /api/llm/status`；mock 模式（用户未配 BYOK 且服务端未启用真实 LLM）时先展示「连接模型服务」欢迎卡——服务商快捷选择（OpenAI/DeepSeek/通义千问，预选进配置抽屉）+ 讲清价值与 Key 存放，保存后自动开始测评（可跳过测评以 A2 起步）；autoStart 等 LLM 状态加载完成再决定（修竞态防 mock 抢先开始）。** **T-066：首次测评完成后结果页下方展示「计划+练习安排」引导（`PlanGuidePanel` 轮询 `GET /api/planner/current`，计划就绪展示概览 +「开始今日练习」跳 /learn），不再直接丢回首页。**
...
- **测评详情收拢（T-069）**：/profile 等级区下方「最近测评」卡只显示当前测评的简要信息（人话标签 + 定级 + 表达力综合分 + 测评日期 + 常见问题 Top 错误标签 + 识别矫正标记），评估报告（能力画像）收进卡内展开（共享组件 `EvaluationReportView`，自 LevelPanel 提取）；「查看测评结果」经 `/assessments?detail=<id>` 深链直达最新测评完整详情（rubric/四维/逐题评语）；`GET /api/assessments` 列表项新增 `rubricLabel`/`topErrorTags` 从 FinalLevel 记录同源投影（零新增 LLM），旧记录缺字段优雅降级。
